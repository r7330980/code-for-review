...
from gennm_model import GenNmDecoder
...
if TYPE_CHECKING:
    ...
    from llmtuner.hparams import ModelArguments, GenNmArguments
...
def load_gennm_classification_model_and_tokenizer(
    model_args: "ModelArguments",
    finetuning_args: "FinetuningArguments",
    gennm_args: "GenNmArguments",
    is_trainable: Optional[bool] = False,
    stage: Optional[Literal["lc"]] = "lc"
    ...
    config.num_classes = gennm_args.class_num
    config.cls_loss_weight = gennm_args.cls_loss_weight
    str2id = {}
    id2str = gennm_args.class_id_to_str
    for k,v in id2str.items():
        str2id[v] = k
    config.id2str = id2str
    config.str2id = str2id
    del gennm_args.class_id_to_str
    config.no_clm = gennm_args.no_clm
    config.mlp_cls = gennm_args.mlp_cls
    ...
    if gennm_args.freeze_llm:
        if 'llama' in model.config.model_type:
            if gennm_args.freeze_layer_num == -1:
                model.model.requires_grad_(False)     
            else:
                model.model.embed_tokens.requires_grad_(False)
                for i in range(gennm_args.freeze_layer_num):
                    model.model.layers[i].requires_grad_(False)            
        elif 'gpt_big' in model.config.model_type:
            if gennm_args.freeze_layer_num == -1:
    ...
    # write trainable params name to file
    if is_trainable:
        with open("dbg-trainable_params.txt", "w") as f:
