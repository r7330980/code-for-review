...
def normalize_in_train_name(name):
...
def prepare_func_str(func_entry, vars_to_rename, max_len=10240):
    ...
    for k, v in func_entry["id_maps"].items():
        if k in vars_to_rename:
            continue
        current_str = replace_variable_names(current_str, k, v)
...
demangled_func_name_pattern = re.compile(
    r".*([^0-9A-Za-z_~]|^)([~0-9a-zA-Z_]+)(<.*>)?(\[abi.*\])?\(.*\)"
)
demangled_operator_name_pattern = re.compile(
    r".*([^0-9A-Za-z_~]|^)(operator ?[^0-9a-zA-Z_]+|new|new\[\]|delete|delete\[\])(<.*>)?(\[abi.*\])?\(.*\)"
)
