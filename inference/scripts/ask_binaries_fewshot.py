...
def parse_args():
    ...
    parser.add_argument(
        "--progs",
        type=str,
        default="dirty-tiny-test-mysplit.preproc.pkl.0",
    )
    ...
    parser.add_argument("--n_example", type=int, default=3)
    parser.add_argument(
        "--example",
        type=str,
        default="fewshot-examples/dirty-train-mysplit.example.jsonl",
    )
    parser.add_argument("--fout", type=str, default="tmp-fewshot.jsonl")
...
class AskManager:
    def __init__(self, config, examples, func, hint_string):
        self.config = config
    ...
    def get_ask_text(self):
        ...
        ids = sorted(list(self.var_id_maps.keys())) + sorted(
            list(self.func_id_maps.keys())
        )
        ask_text = "\n\n"
        if self.hint_string != "":
    ...
    def skip(self, n):
        examples = random.sample(self.examples, n)
        return examples

    def ask_fewshot(self, n):
        sys_prompt = """
...
{"var1": "suggested_name1", "var2": "suggested_name2", ...}
        ...
        if n > 0:
            ...
            for example in examples:
                question = example["ask"]
                answer = example["answer"]
                message.append({"role": "user", "content": question})
                message.append({"role": "assistant", "content": answer})

        message.append({"role": "user", "content": self.get_ask_text()})
...
def main():
    ...
    if args.hints != "":
        ...
        hints = {}
    examples = [json.loads(l) for l in open(args.example, "r")]
    ...
    for prog in progs:
        ...
        for func_name, func_entry in prog.stripped_name2func.items():
            ...
            ask_manager = AskManager(
                ask_config, examples=examples, func=func_entry, hint_string=hint_string
            )
            pbar.update(1)
            if instance_cnt < args.from_idx:
                instance_cnt += 1
                ask_manager.skip(args.n_example)
                continue
            if instance_cnt >= args.to_idx:
                done=True
                break
            instance_cnt += 1
            ret = ask_manager.ask_fewshot(args.n_example)
