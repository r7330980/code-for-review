...
def parse_args():
    ...
    args.add_argument(
        "--bin_list",
        type=str,
        default="test-bin-list.txt",
    )

    args.add_argument("--names", type=str, default="")
    args.add_argument("--fout", type=str, default="new-names.jsonl")
    args.add_argument(
        "--model_in", type=str, default=""
    )
    args.add_argument(
        "--tokenizer", type=str, default="codellama/CodeLlama-7b-Instruct-hf"
    )
    args.add_argument("--prop_round", type=int, default=1)
    ...
    def receive_propagation_from(self, another, prop_reason=None):
        for entry in another.propagation_list:
            if self.has_prop_entry(entry):
                continue
            # if prop_reason is not None and entry.prop_reason != prop_reason:
            #     continue
            other_entry_copied = copy.deepcopy(entry)
            other_entry_copied.prop_level += 1
            # if prop_reason is not None:
            #     other_entry_copied.prop_reason = prop_reason
...
def _from_callee_args(
    ...
    all_call_exprs = [expr for expr in all_exprs if type(expr) == gennm_ir.GenNmCallExpr]
    for call_expr in all_call_exprs:
        ...
        callee_params = callee_parsed.args
        ...
        for i in range(len(callsite_args)):
            ...
            if (
                type(callsite_arg) != gennm_ir.GenNmVarExpression
                and type(callsite_arg) != gennm_ir.GenNmImplicitReturnVarExpr
            ):
                continue
            current_fully_qualified_name = (
                current_prog_name,
                current_func_parsed.func_id,
            ...
            if current_fully_qualified_name not in propagation_records:
                propagation_records[current_fully_qualified_name] = PropagationRecorder(
                    ...
                    func_name=current_func_parsed.func_id,
            ...
            #     callee_prop_recorder = propagation_records[prop_fqn]
            #     propagation_records[
            #         current_fully_qualified_name
            #     ].receive_propagation_from(
            #         callee_prop_recorder, prop_reason="from callee args"
            #     )
...
def _from_callee_return(
    ...
    for expr in all_exprs:
        if type(expr) != gennm_ir.GenNmBasicExpr:
            continue
        ...
        if len(expr.defs) == 0:
            continue
        use = expr.uses[0]
        if type(use) != gennm_ir.GenNmImplicitReturnVarExpr:
            continue
    ...
    for expr_to_analyze in interesting_exprs:
        ...
        callee_return_exprs = [
            expr for expr in callee_exprs if type(expr) == gennm_ir.GenNmReturnStmt
        ]
        ...
        for ret_expr in callee_return_exprs:
            if type(ret_expr.ret_val) != gennm_ir.GenNmVarExpression:
                continue
...
def _from_caller_args(binary_prog, current_func_parsed, names, propagation_records):
    if current_func_parsed.func_id not in binary_prog.call_graph:
        return {}
    callers = list(binary_prog.call_graph.predecessors(current_func_parsed.func_id))
    if len(callers) == 0:
        return {}
    my_func_id = current_func_parsed.func_id
    for caller in callers:
        ...
        caller_call_exprs = [
            expr for expr in caller_exprs if type(expr) == gennm_ir.GenNmCallExpr
        ]
        ...
        for callsite in callsites:
            ...
            my_params = current_func_parsed.args
...
def _from_caller_return(binary_prog, current_func_parsed, names, propagation_records):
    ...
    my_return_exprs = [
        expr for expr in my_exprs if type(expr) == gennm_ir.GenNmReturnStmt
    ]
    interesting_my_return_vars = []
    my_ret_var_set = set()
    for expr in my_return_exprs:
        if type(expr.ret_val) != gennm_ir.GenNmVarExpression:
            continue
    ...
    for caller in callers:
        ...
        for expr_to_analyze in interesting_caller_exprs:
            ...
            for ret_var in interesting_my_return_vars:
                ...
                # if prop_fqn in propagation_records:
                #     prop_source_recorder = propagation_records[prop_fqn]
                #     propagation_records[current_fqn].receive_propagation_from(
                #         prop_source_recorder, prop_reason="from caller return"
                #     )
...
def _among_direct_use(
    ...
    for expr in all_exprs:
        ...
        if len(expr.defs) == 0:
            continue
        use = expr.uses[0]
        if type(use) != gennm_ir.GenNmVarExpression:
            continue
        interesting_exprs.append(expr)
    for expr_to_analyze in interesting_exprs:
        defined_var_name = expr_to_analyze.defs[0].var_name
        used_var_name = expr_to_analyze.uses[0].var_name
        rhs_fqn = (current_prog_name, current_func_parsed.func_id, used_var_name)
        lhs_fqn = (current_prog_name, current_func_parsed.func_id, defined_var_name)
        if prop_from_rhs:
            prop_fqn = rhs_fqn
            receive_fqn = lhs_fqn
        else:
...
def _filter_new_names(new_names):
    ...
    for k, v in new_names.items():
        ...
        for one_prop_source in prop_preds:
            # if len(one_prop_source) >= 2:
            #   # if cls model and gen model agree on the name
            #   if one_prop_source[0]['pred_name'] == one_prop_source[1]['pred_name']:
            #       confident_props.append(one_prop_source[0]['pred_name'])
            #       continue
            name_cnt = {}
            cnt = 0
...
def main():
    args = parse_args()
    tokenizer = CodeLlamaTokenizerFast.from_pretrained(args.tokenizer)
    embeddings = torch.load(args.model_in + "/name_embeddings.pt").cuda()
    ...
    binary_list = open(args.bin_list, "r").readlines()
    binary_list = [l.strip() for l in binary_list]
    data = []
    for binary_fname in tqdm(binary_list, desc="Loading binaries"):
        current_bin = pickle.load(open(binary_fname, "rb"))
        data.extend(current_bin)
    time_before_prop = time.time()
    # pickle.load(open(args.binary, "rb"))
    new_names = {}
    prop_stats = {}
    propagation_records = {}
    ...
    for k, v in tqdm(names.items(), desc="Selecting new names"):
        ...
        if len(name_candidates) == 0:
            ...
            # selected = v["name_list"][0]
            new_name_selections[k] = selected
            continue
        name_tokens = [
            torch.tensor(tokenizer.encode(name)).cuda() for name in name_candidates
        ]
        with torch.no_grad():
            name_embs = [
                torch.mean(embeddings(name_token), dim=0) for name_token in name_tokens
            ]
            name_embs_tensor = torch.stack(name_embs)

        if k not in filtered_new_names:
            selected = default_names[k]
            # selected = v["name_list"][3]
            new_name_selections[k] = selected
            continue
            ...
            #     new_name_selections[k] = selected
            # else:
            #   name_embs_norm = name_embs_tensor / name_embs_tensor.norm(dim=-1, keepdim=True)
            #   self_similarity = torch.matmul(name_embs_norm, name_embs_norm.transpose(0,1))
            #   score = torch.sum(self_similarity, dim=-1)
            #   sorted_scores = torch.argsort(score, descending=True)
        ...
        prop_name_tokens = [
            torch.tensor(tokenizer.encode(name)).cuda() for name in names_for_voting
        ]
        with torch.no_grad():
            prop_name_embs = [
            ...
            selected = name2name_list_entry[name_candidates[sorted_scores[0]]]
            new_name_selections[k] = selected
            # default_name = default_names[k]['pred_name']
            # name2score = {}
            # for i in range(len(name_candidates)):
            #     name2score[name_candidates[i]] = scores[i].item()
            # if default_name not in name2score:
            #     if len(name2score) < 2:
            #         score_diff = 0
            #     else:
            #         score_diff = scores[sorted_scores[0]].item() - scores[sorted_scores[1]].item()
            #     default_not_in += 1
            # else:
            #     score_diff = scores[sorted_scores[0]].item() - name2score[default_name]
            # if score_diff < 0:
            #     selected = default_names[k]
            #     new_name_selections[k] = selected
            # else:
