...
def craft_binary_prog_from_varbert(prog_name, varbert_program):
    # map from func name to dataset entries
    ret_prog = BinaryProgram(prog_name, [])
    del ret_prog.stripped_name2entry
    del ret_prog.funcs_all
    del ret_prog.funcs_filtered
    for k, v in varbert_program.items():
        ret_prog.stripped_name2func[k] = v
    for func_name in varbert_program.keys():
        if func_name is None:
            continue
        ret_prog.call_graph.add_node(func_name)
    for func_name, func_entry in varbert_program.items():
        body = func_entry.body
        if func_name is None:
            continue
        for match in BinaryProgram.FUNC_PATTERN.finditer(body):
            callee_name = match.group(2)
            if callee_name == func_name:
                continue
            if callee_name in varbert_program:
                ret_prog.call_graph.add_edge(func_name, callee_name)
    return ret_prog